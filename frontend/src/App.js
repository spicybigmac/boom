...
function App() {
    ...
    const processVodLink = async (vodLink) => {
        setError(''); // clear any previous errors
        setView('loading'); // switch to the loading screen
        ...
        try {
            ...
            // if the backend returns an error (like 404 or 422)
            if (!response.ok) {
    ...
    const renderView = () => {
        switch (view) {
            case 'loading':
                return <LoadingPage />;
            case 'results':
                return <ResultsPage data={resultsData} interval={interval} onGoBack={handleGoBack} />;
            case 'home':
            default:
                return <HomePage 
                          onProcessLink={processVodLink} 
                          error={error} 
                          interval={interval}
